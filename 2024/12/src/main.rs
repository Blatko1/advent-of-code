...
fn part1(input: &str) -> usize {
    ...
    while let Some((start_x, start_y, plot)) = find_next_field(&map) {
        let plot_fields = Dfs::new(&mut map, plot).search(start_x as i32, start_y as i32);
    ...
    while let Some((start_x, start_y, plot)) = find_next_field(&map) {
        ...
        for (x, y) in plot_fields {
            if y as i32 - 1 >= 0 {
                let adjacent_plot = map[y-1][x].2;
                if adjacent_plot != plot {
                    fences.push((x as i32, y as i32-1, Orientation::Horizontal, AttachedTo::Down));
                }
            } else {
                fences.push((x as i32, y as i32-1, Orientation::Horizontal, AttachedTo::Down));
            }
            if y as i32 + 1 < max_height {
                let adjacent_plot = map[y+1][x].2;
                if adjacent_plot != plot {
                    fences.push((x as i32, y as i32+1, Orientation::Horizontal, AttachedTo::Up));
                }
            } else {
                fences.push((x as i32, y as i32+1, Orientation::Horizontal, AttachedTo::Up));
            }
            if x as i32 - 1 >= 0 {
                let adjacent_plot = map[y][x-1].2;
                if adjacent_plot != plot {
                    fences.push((x as i32-1, y as i32, Orientation::Vertical, AttachedTo::Right));
                }
            } else {
                fences.push((x as i32-1, y as i32, Orientation::Vertical, AttachedTo::Right));
            }
            if x as i32 + 1 < max_width {
                let adjacent_plot = map[y][x+1].2;
                if adjacent_plot != plot {
                    fences.push((x as i32+1, y as i32, Orientation::Vertical, AttachedTo::Left));
                }
            } else {
                fences.push((x as i32+1, y as i32, Orientation::Vertical, AttachedTo::Right));
            }
...
fn remove_horizonal_dir(fences: &mut Vec<(i32, i32, Orientation, AttachedTo)>, x: i32, y: i32, direction: i32, attached_to: AttachedTo) {
    if let Some(idx) = fences.iter().position(|f| f == &(x+direction, y, Orientation::Horizontal, attached_to)) {
...
fn remove_vertical_dir(fences: &mut Vec<(i32, i32, Orientation, AttachedTo)>, x: i32, y: i32, direction: i32, attached_to: AttachedTo) {
    if let Some(idx) = fences.iter().position(|f| f == &(x, y+direction, Orientation::Vertical, attached_to)) {
...
fn find_next_field(map: &[Vec<(usize, usize, char)>]) -> Option<(usize, usize, char)> {
    for row in map.iter() {
        if let Some(&field) = row.iter().find(|(_, _, plot)| !plot.is_ascii_lowercase()) {
            return Some(field)
        }
    }
    None
