...
fn part1(input: &str) -> u32 {
    ...
    for (_, [pg_before, pg_after]) in
        rules_regex.captures_iter(input).map(|c| c.extract())
    {
        let before: u32 = pg_before.parse().unwrap();
        let after: u32 = pg_after.parse().unwrap();
        page_map.entry(before).or_default().insert(after);
    }
    for update_line in input
        .lines()
        .skip_while(|line| !line.trim().is_empty())
        .skip(1)
    {
        let updates: Vec<u32> = update_line
            .split(',')
        ...
        for (current, &page) in
            updates.iter().enumerate().take(update_count - 1)
        {
            let Some(succeding_pages) = page_map.get(&page) else {
                is_valid = false;
                break;
            };
            if updates[current + 1..update_count]
                .iter()
                .any(|pg| !succeding_pages.contains(pg))
            {
                is_valid = false;
            }
        }
...
fn part2(input: &str) -> u32 {
    ...
    for update_line in input
        ...
        .skip(1)
    {
        let mut updates: Vec<u32> = update_line
            .split(',')
            .map(|page| page.parse::<u32>().unwrap())
            .collect();
        let update_count = updates.len();
