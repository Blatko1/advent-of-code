...
fn part1() {
    ...
    for line in input.lines() {
        ...
        let split: Vec<_> = content.split('|').map(|nums| nums.trim()).collect();
        let winning_numbers: Vec<u32> = split[0].split_whitespace().map(|num|num.parse().unwrap()).collect();
        let mut available_numbers: Vec<u32> = split[1].split_whitespace().map(|num|num.parse().unwrap()).collect();
